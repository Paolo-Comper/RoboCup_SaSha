import SeguiLinea
from Test.RasbyTest.main import Visualizza
import cv2
import numpy as np
...
def setup_camera():
    ...
    picam2.set_controls({
        "AeEnable": False,  # Disabilita l'auto esposizione
        "ExposureTime": 10000,  # Aumenta l'esposizione per una maggiore luminosità
        "AnalogueGain": 24.0,  # Aumenta il guadagno per compensare la bassa luminosità

        # Bilanciamento del bianco
        "AwbEnable": True,  # Usa il bilanciamento automatico del bianco
        "AwbMode": 0,  # Modalit� automatica

        # Flickering (50Hz per Europa)
        "AeFlickerMode": 1,  
        "AeFlickerPeriod": 20000,
...
def line_processing():
    ...
    for j in range(N_CERCHI):
        ...
        if np.count_nonzero(semicerchio_grad):
            ...
            #? Verdi
            p_angolo = SeguiLinea.greenFollowing(punto, p_angolo)
...
def line_following():
    ...
    return (distanza_linea * KE) + (direzioni[0] * K1) + ((direzioni[1] - direzioni[0]) * K2) + ((direzioni[2] - direzioni[1]) * K3)
